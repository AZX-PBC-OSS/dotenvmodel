...
!!! note "`str` defaults are interpolation templates"
    `${VAR}` and `${VAR:-default}` references in a `str` default resolve at load time against the fully merged `.env` cascade over the process environment — every merged key is visible to every reference, unlike `.env` values themselves, which resolve progressively in merged-key order. `default_factory` results are never interpolated, and resolution re-runs on every `load()` / `reload()`. See [Variable Interpolation](loading.md#variable-interpolation) for the full semantics.
